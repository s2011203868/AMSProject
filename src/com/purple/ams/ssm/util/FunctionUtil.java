...
public class FunctionUtil {
	...
	/**
	 * 
	* @Title: randomSixCharStr  
	* @Description: 返回6个随机字符组成的字符串 
	* @param @return    设定文件  
	* @return String    返回类型  
	* @throws
	 */
	public static String randomSixCharStr(){
		String uuid= UUID.randomUUID().toString().replaceAll("-","").trim();
		String [] randomeStr32 = uuid.split("");
		StringBuffer sBuffer = new StringBuffer();
		for(int i = 0 ; i< 6 ;i++){
			int index = new Random().nextInt(randomeStr32.length);
			sBuffer.append(randomeStr32[index]);
		}
		String randomStr = sBuffer.toString().trim();
		return randomStr;
	}
	...
	* @throws
	 */
	public static String md5Str(String password,String salt,int count){
